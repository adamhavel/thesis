...
In the end, the resource returns a collection of all the blueprints in the database that satisfy the inherent or supplied conditions. Anyhow, the view on the collection is shallow, meaning only identifiers of a blueprint are returned, not an actual content. Since on the client side the resource is used only for listings, anything more than that would be a waste of bandwidth. For these reasons, it responds only to request of the \code{GET} type.
